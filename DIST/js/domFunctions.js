export const setPlaceholderText = () =>{
    ...
    (window.innerWidth < 400) ? (input.placeholder = "City, State, country") : (input.placeholder ="City, State, Country, Zipcode");
}
...
const buildScreenReaderWeather = (weatherJson, locationObj) =>{
    ...
    return `${weatherJson.current.weather[0].description} and ${Math.round(Number(weatherJson.current.temp))}º${tempUnit} in ${location}`;
};
...
const createCurrentConditionsDivs = (weatherObj, unit)=>{
    ...
    const temp = createElem("div", "temp", `${Math.round(Number(weatherObj.current.temp))}º`, tempUnit);
    ...
    const feels = createElem("div", "feels", `Feels Like ${Math.round(Number(weatherObj.current.feels_like))}º`);
...
const createDailyForecastDivs = (dayWeather) =>{
    ...
    const dayHigh = createElem("p", "dayHigh", `${Math.round(Number(dayWeather.temp.max))}º`);
    const dayLow = createElem("p", "dayLow", `${Math.round(Number(dayWeather.temp.min ))}º`);
